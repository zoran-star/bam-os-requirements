...
- **Four things come back, not one.** `needs` (currently_due + past_due), `reviewing` (pending_verification), `problems` (`requirements.errors`, Stripe's own sentence about something already submitted and rejected) and `disabled_reason`. `problems` is the one people forget: without it an owner whose ID upload was rejected just sees the same item asked for again and re-uploads the same file.
