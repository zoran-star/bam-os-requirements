...
| 2026-07-09 | HOME WIREFRAME (Zoran): two KPI cards on top - Trials today with the LIST of trial names (times + athletes) · Qualified trial close rate + Sales in the past week with the sale-name rows - then ONE pipeline panel: full-width gold HAWKEYE bar (count + top action; gold fill when actions wait), the pill strip, lowkey recent movement. Hawk bench + hawk card removed |
| 2026-07-09 | WIRING AUDIT: the home Hawkeye count only queried Booking + the retired followups - Confirm/Closing never showed. _ccSalesHawk now merges all 3 list-ready queues (closing followup_N rows group per contact). Trials today verified: calendars-v15 trials-today routes portal-provider academies (GTA) to trial_bookings + schedule_slots - correct source, now renders the full list |
| 2026-07-09 | STANDALONE SALES PAGE RETIRED (V2): Sales = the home section. Hawkeye + per-stage config open as mm-focus overlays (#salesMachineModal, same warp as the marketing section; #pl-hawkeye/#pl-focus reparented into it). All entries repointed (_cc2Lead/_ccPipeFocus/_hv2OpenHawkeye direct); cascade "+N more" now EXPANDS IN PLACE (_plo2ShowAll) - the board is no longer a destination on V2. V1.5 untouched |
| 2026-07-09 | POST-TRIAL FORM = a Hawkeye card on the Confirm tab: agent-confirm list-ready synthesizes kind=post_trial rows (portal academies: trial_bookings BOOKED + slot in the past 7 days + no post_trial_reviews row + open opp). Deck renders the form (showed up / good fit / first message / sign-up link / notes) and submits through /api/ghl/post-trial - same 3-way routing. No Other button on form cards |
| 2026-07-09 | 'Liked' RULE: an inbound message starting with the exact text "Liked" (iMessage/IG tapback) never registers - filtered in isRealInbound (agent/_stage.js) across all 3 agent queues + the portal SMS overlay, the GHL reply webhook (no draft-cancel, no Ghosted/Nurture bounce), the Meta inbound store (never written), and every agent thread builder |
| 2026-07-09 | Section titles BIG (cc-title 24 -> 44px, weight 900) across every cc section; the "Pipeline, agents and trials." Sales subtitle removed. Agent pills LIGHT UP gold (glow pulse) while a Hawkeye action waits (.plo2-pill.needs when sig.hawk > 0) |
| 2026-07-09 | ACCURACY SCAN: reconciliation test proves the home HAWKEYE bar count == the deck's actual card count on the same data - approved-status rows dropped, closing followup_N rows collapse to ONE plan per contact, post-trial form counted once. Both read the same 3 list-ready queues. (Live GTA data still to be spot-checked in prod.) |
| 2026-07-09 | DETECTOR STARVATION BUG FIXED (all 3 agents): each detector `slice(0, DETECT_CAP=10/12)` on a NEWEST-FIRST queue - once the top N cards were carded, every quiet lead past position N NEVER got a card (Done-Trial leads sat with us as last message, nothing queued). Fix: (closing) filter out proactive leads that already have a pending/approved card + order the rest LONGEST-SILENT first so the cap lands on real work + drains over runs; (confirm) filter carded leads only (order left - it's appointment-time driven); (ghost) skip carded + mid-intro leads in the candidate builder so the cap only counts leads that need a new card. Stable order + skip-carded = the frontier advances each 15-min run until the whole backlog is queued. |
| 2026-07-09 | REMAINING stuck buckets are CONFIG (not code): a custom-named stage that matches no role regex has no engine; Interested leads whose intro finished while Ghosted automation is OFF; quiet Responded coverage needs booking agentMode ON + a configured ghosted_workflow. Verify these on GTA for true full-confidence. |
