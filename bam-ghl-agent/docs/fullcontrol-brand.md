...
Design principles that shape how Full Control interfaces are built. These sit alongside the visual system — they govern layout decisions, information hierarchy, and interaction patterns.

---

**UX-01 · Place guiding information at the point of action.**

Guiding text — labels, prompts, instructions, sub-headings — belongs immediately adjacent to the element it describes. Never place a cue above an intervening section, blank gap, or unrelated block and expect the user to connect it to a destination further down the page. The instruction and its target should be visually inseparable.

*In practice:* A label that says "Open a new request" earns its place directly above the request cards, not in the page header above a divider. A "Track what's in flight" prompt belongs above the ticket list, not a scroll away.

---

**UX-02 · Minimize eye travel.**

Every pixel of distance the eye must travel between a label and its content is friction. Reduce it. Pair prompts with their targets. Avoid layouts where the user must scan across a wide gap, scroll past blank space, or parse an unrelated element to connect a cue to its destination. If two things belong together conceptually, they should be together spatially.

*In practice:* Prefer placing sub-labels in-line with or immediately above their section rather than in a page-level topbar. The shorter the path from "what does this mean?" to "where do I act?" — the better.

---

## 10 · Light Mode

Full Control is dark-first. Light mode is a user-selectable surface inversion — not a separate design language. The feeling stays the same: commanding, precise, quiet. Only the surfaces flip.

### When to use

Light mode suits client-facing portals, print exports, and any context where an ink background feels heavy (e.g. daylight use, external presentations). Internal tools default dark.

### Surface mapping

| Role | Dark token | Light value | Hex |
|---|---|---|---|
| Page background | `--ink` | `--paper` | `#F5F1E8` |
| Raised surface | `--surface` (`--ink-2`) | `--paper-2` | `#EBE4D3` |
| Cards / elevated | `--surface-el` (`--ink-3`) | `--paper-3` | `#E0D7C1` |
| Hover state | `--surface-hov` | `--paper-3` darker | `#D4C9AD` |
| Hairline divider | `--line-dark` | `--line-light` | `#D4C9AD` |
| Strong divider | `--line-dark-2` | `--line-light-2` | `#B8A77F` |
| Primary text | `--text` (paper on ink) | ink on paper | `#0A0A0B` |
| Secondary text | `--text-sub` | `rgba(10,10,11,0.65)` | — |
| Muted text | `--text-mute` | `rgba(10,10,11,0.38)` | — |

### Gold discipline in light mode

Signal Gold (`#E8C547`) is unreadable as text on paper. In light mode:

- **Gold text / labels** → use `--gold-3` (`#8B6F2A`) — Deep Gold, AA-compliant on paper
- **Gold dots / live indicators** → use `--gold-2` (`#C9A55C`) — Champagne, still legible
- **Gold fills (buttons, active nav bg)** → `--gold-3` text on `--gold` fill still works
- **Accent ghost / border** → shift opacity down: `rgba(139,111,42,0.08)` / `rgba(139,111,42,0.22)`

### Implementation

Use a `data-theme` attribute on `<html>` to toggle. All tokens are overridden — components require no structural changes.
...
html[data-theme="light"] {
  ...
  --line-str:     #B8A77F;
  --text:         #0A0A0B;
  --text-sub:     rgba(10,10,11,0.65);
  --text-mute:    rgba(10,10,11,0.38);
  --gold:         #8B6F2A;   /* Deep Gold — AA on paper */
  --gold-2:       #C9A55C;   /* Champagne — dots, secondary */
  --accent-ghost: rgba(139,111,42,0.08);
  --accent-border:rgba(139,111,42,0.22);
}
```
...
- Typography scale, weights, tracking — identical in both modes
- Component structure — no layout changes
- Pace colors — muted enough to work on both surfaces without adjustment
- Corner radii, spacing, grid — unchanged
- Voice and microcopy — unchanged

### Light mode checklist

Before shipping a light-mode screen:

- [ ] Background is `--paper` (`#F5F1E8`), not pure white
- [ ] All gold text uses `--gold-3` (`#8B6F2A`), not Signal Gold
- [ ] Dividers are `--line-light` (`#D4C9AD`), still 1px solid
- [ ] Active nav uses `rgba(139,111,42,0.08)` ghost, not the dark-mode gold ghost
- [ ] No shadows added "to make things pop on white" — hairlines only
- [ ] Pace colors unchanged — check contrast on paper if in doubt
- [ ] Toggle button is always accessible regardless of scroll position
