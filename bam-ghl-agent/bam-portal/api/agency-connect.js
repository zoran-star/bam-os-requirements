...
async function exchangeCode(code, redirect) {
  const r = await fetch(GHL_TOKEN_URL, {
    ...
    body: new URLSearchParams({
      client_id: (process.env.GHL_AGENCY_OAUTH_CLIENT_ID || "").trim(),
      client_secret: (process.env.GHL_AGENCY_OAUTH_CLIENT_SECRET || "").trim(),
...
async function handler(req, res) {
  ...
  if (action === "start") {
    ...
    const params = new URLSearchParams({ client_id: (process.env.GHL_AGENCY_OAUTH_CLIENT_ID || "").trim(), redirect_uri: redirectUri(req), scope: SCOPES, state });
