...
| `20260730T160000_locations_entry_note.sql` | Adds nullable `locations.entry_note` (additive, no policy change), then seeds ONE row: BAM GTA's Linbrook venue (1079 Linbrook Rd) gets "The gym entrance we use is at the front of the building, on the left side." Guarded on `entry_note IS NULL` so a re-run cannot overwrite a later owner edit. Every other venue stays NULL, including GTA's second venue (Mildred's) and both San Jose venues | Nothing 500s and nothing is held, but until it is applied NO academy sends an entry sentence at all - including GTA, which loses the one it had. That is the intended degradation: the hardcoded sentence is GONE from the code either way, so the choice is "GTA is briefly missing its door line" vs "26 academies keep describing a door in Oakville". The Business Blueprint > Locations field also stays read-only-ish until then: it renders and accepts input, and the guarded save refuses to write the column while it is absent from the loaded row rather than 400ing the update | 2026-07-30 |
