...
|---|---|---|---|
| `20260807T160000_v2_tickets_system_key.sql` | Adds `v2_tickets.system_key text` (nullable, no backfill) + unique index `v2_tickets_client_system_key_uk` on `(client_id, system_key)`. Mirrors `action_items.system_key` (20260807T140100). It IS the idempotency of the member-apply deferred-mint queue (decision E): one BAM systems-lane `billing_fix` ticket per (client, offer, family, amount), so a rerun of apply cannot duplicate it | Nothing breaks unapplied: `createSystemTicket` in `api/v2-tickets.js` catches the missing-column error (42703/PGRST204) and degrades to a read-then-write dedupe on `intake.system_key`, so the mint ticket still queues and still dedupes - just without the hard unique-index guard until this runs. Additive and unread by any other path, so applying first is inert | 2026-08-07, member-apply mint-queue build. Written not applied (remote session) - for `/pending-sql` |
