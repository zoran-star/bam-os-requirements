-- When a client enters their website in the self-serve onboarding flow
-- (onboarding_reloaded.answers, key 'website_1' = "Existing website URL"),
-- mirror it into clients.brand_data.website_url — the one field Cam's
-- marketing Client card reads. Fills only when empty so a staff-curated
-- value is never clobbered. SECURITY DEFINER so the anon onboarding upsert
-- can trigger the clients update.
