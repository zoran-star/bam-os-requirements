...
-- website /api/website/assets endpoint.
...
create table if not exists public.client_assets (
  ...
  label        text,
  category     text not null default 'other',   -- logo|wordmark|hero|photo|crest|icon|og|favicon|other
  alt          text,
...
comment on table public.client_assets is 'Per-academy brand/asset library. Each image optionally tagged to an offer, staff member, or location. Stored in the client-assets bucket at <client_id>/<...>.';
...
-- Storage policies mirror the 'offers' bucket: public read, client-scoped writes by path prefix.
