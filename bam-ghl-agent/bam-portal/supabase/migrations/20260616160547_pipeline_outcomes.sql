-- V1.5 Pipelines: capture a free-text reason when a card is marked
-- won / lost / abandoned (the GHL opportunity status is still updated; this
-- stores the reason for the record).
create table if not exists public.pipeline_outcomes (
  id uuid primary key default gen_random_uuid(),
  client_id uuid not null references public.clients(id) on delete cascade,
  opportunity_id text not null,
  status text not null,            -- won | lost | abandoned | open (undo)
  reason text,
  created_by uuid,
