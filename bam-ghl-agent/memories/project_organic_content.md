...
⚠️ **And a THIRD time on the `tickets` table** (2026-07-12): the funnel send-to-systems handoff inserted priority='normal' + source='funnel-content' - but `tickets` CHECKs allow priority (urgent|standard|low), source (portal|asana_import), category (systems|website|ads|other|null). First live use 400'd on Cam; fixed to standard/portal/website + the blocked handoff was completed manually via SQL (systems ticket a1963a43). **Rule extended: any INSERT into a table you don't own in that PR - verify its CHECK constraints first (pg_constraint), don't invent enum values.**

⚠️ **Same gotcha bit AGAIN on `content_tickets.client_action_status`** (2026-07-06): the content-approval-gate feature (2026-06-26) started writing `'review-requested'` in send-for-review but never rebuilt `content_tickets_client_action_status_check` - so EVERY "Send for client review" (organic + gated ads) failed with 23514 for ~10 days until Eli hit it. Fixed in `20260706175523_content_tickets_review_requested_status.sql` (applied). **Rule: any new enum-ish value on content_tickets (status, client_action_status, type) needs its CHECK constraint rebuilt in the same PR.**
