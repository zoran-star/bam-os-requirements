...
**2026-07-14: Systems -> Content -> Systems round trip (Zoran approved via text).** Staff can now CREATE content tickets on a client's behalf: POST content-tickets with `client_id` (+ optional `origin_systems_ticket_id`) from any staff caller - detected by explicit client_id so staff who are also client members don't file against their own academy. Skips organic credits + the funnel plan gate. context gets `source='staff-request'`, `requested_by_staff_id/name`. Entry point: "Request content" button in SystemsView's ticket action bar (inline Section, type+notes, channel=funnel, prefills from fields.what). ContentView shows a "from Systems · CODE" chip; for origin-linked tickets the send button becomes **Return to Systems** (`return-to-systems` action): finals APPEND to the origin systems ticket's files (reopening cancelled/done origins), internal message added, requester gets a Slack DM - no duplicate ticket. Interim conversion (Prime By Design 0A2) is origin-linked so its return leg works too.

**2026-07-09: Client media library (staff).** `?resource=client-media&client_id=` (GET, CONTENT_ROLES) aggregates raw_files across ALL of a client's content tickets + the "• name - url" attachment bullets from client action-response messages. Rendered as a collapsible `ClientMediaLibrary` card in ContentTicketDetail (lazy-fetch on expand, folder groups, video/image filter, search, origin ticket code, gold border = current ticket). Solves "use the b-roll I previously sent" - no migration, files were always retained in ticket-files; this is discovery. **2026-07-15 upgrade:** real thumbnails (images via `ctkThumbUrl` Supabase transform w/ fallback; videos via `CmVideoThumb` = lazy IntersectionObserver-mounted `<video preload="metadata" src="url#t=0.5">` poster frame) + multi-select checkboxes with Select all (visible) / zip download via the existing `ctkDownloadZip` helper. Selection keyed by file url and survives filter changes; external-link files that fail CORS fetch are silently skipped by the zipper. **Download routing (2026-07-16):** `ctkDownloadPicked` zips only small non-video batches (<= 12 files, no videos); anything bigger streams each file directly to disk via Supabase storage `?download=<name>` URLs (`ctkDownloadDirect`, 350ms stagger, one Chrome multi-download prompt) with live `Downloading i/n…` progress. In-browser JSZip buffers everything in RAM and can die at Chrome's ~2GB blob ceiling - never route video batches through it. Big batches now prefer `ctkDownloadToFolder` (File System Access API `showDirectoryPicker`, Chrome/Edge): pick a folder once, files stream network->disk via `pipeTo` - immune to Chrome's 'ask where to save each file' setting, which made per-file anchor downloads pop 78 Save As dialogs for Cam. Anchor method stays as the non-Chromium fallback. Picker is keyed `id: clientId` (sanitized to 32 chars) so Chrome remembers the last-used folder per client. **Selection UX (2026-07-16):** folder-group header checkbox (indeterminate when partial) + shift-click range select in rendered order (anchor = last clicked tile, range takes the clicked tile's new state; `_cmAnchorRef` + `flatOrder`/`idxOf` maps in `ClientMediaLibrary`). shiftKey read via the input's onClick (change events don't carry modifiers). Same-day follow-up: clicking an image/video tile opens an in-app lightbox player (`preview` state, Esc/backdrop close, Download link) - navigating to a raw .MOV URL made Chrome download instead of play. Now SHARED: `src/components/MediaLightbox.jsx` (component) + `src/lib/media.js` (mlIsVideo/mlIsImage/mlIsMedia detection, mime + extension fallback) - wired into ClientMediaLibrary, ContentView `FilePreviewTile` (client inputs/finals grids), and MarketingView `SubmittedFileTile` (ticket detail attachments). Pattern: lightbox renders as a SIBLING of the <a> tile (never inside it). **Promote raw -> finals (2026-07-16):** select raw client files in the ticket detail -> '-> Use N as finals' (FilesByFolder `onPromote` prop -> `promoteToFinals` in ContentTicketDetail -> existing `upload-final` PATCH). Copies the storage reference (raw `type` maps to finals `mime`), dedupes by url, zero download/re-upload - for clients who send ready-to-go creatives. Link entries (text/uri-list) ARE promotable (2026-07-27) - they get a checkbox when onPromote is present even though download/zip still skips them; the toolbar's promote button counts from `promotable` (any url), download from `dlList` (files only). ALL Download controls (lightbox header + tile captions) must use `mlDownloadUrl(f)` (lib/media.js, appends storage `?download=<name>`) - the raw URL renders images inline in a tab instead of downloading, and the anchor `download` attribute is ignored cross-origin.
