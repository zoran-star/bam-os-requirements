...
**Data-quality round 2 (Zoran-approved, shipped 2026-07-16):** (1) `members.joined_date` corrected from earliest paid invoice via `scripts/fix-member-join-dates.mjs` - SIBLING-SAFE: members sharing a parent's stripe_customer only move if an invoice matches their OWN sub; 6 moved, 6 ambiguous siblings left as-is. (2) Duplicate cancel rows deduped (4 deleted) + partial UNIQUE indexes (one cancel per sub / per subless-member) + check-before-insert in actionCancel. (3) Period-end cancels stamp Stripe `current_period_end` as `cancel_date`, not the button-press day. (4) `reason_category` column (schedule|price|injury|moved|service|seasonal|other) + dropdown in the staff cancel modal; parent app can pass `reason_category` in the cancel body when its UI adds it. Rejoin/win-back linkage = still future.
