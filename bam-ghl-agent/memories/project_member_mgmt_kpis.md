...
| MRR | live + cancelling, paused excluded (existing `_ccMemberKpis` behavior) |
...
2. **`_ccMonthly` (client-portal.html) is the SINGLE MRR engine** - Shield KPIs, Home dashboard MRR, Hawkeye Mission Control, growth sparklines, ARPU all run through it. Before #1466 it ignored terms: 3-month prices counted 3x over, 6-month 6x over, 4_weeks ~8% under, one_time counted as recurring. Fixed to decode `N_unit` generically + zero out one_time. If you add a new term to `termToInterval`, make sure `_ccMonthly`'s regex still decodes it.
