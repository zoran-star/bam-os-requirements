...
- [Member Management → Client Portal](project_member_management_portal.md) — 2026-05-20: incorporating the BAM GTA member-management system into the client portal as a client-side "Members" feature. Stripe model = Connect; Phases 1a/2a/2b done — Members tab built end to end; schema SQL (`bam-portal/supabase/member-management-schema.sql`) not yet run. Resume via `/member-management-continue`.
- [Multi-User Client Portal Access](project_multi_user_portal.md) — 2026-05-20: many logins per academy via `client_users` join table. DB foundation + RLS rewrite APPLIED to live Supabase; API + both portal UIs still to build. Paused on an OPEN decision (wide-open staff RLS policies). **Resume via `/account-continue`.**
