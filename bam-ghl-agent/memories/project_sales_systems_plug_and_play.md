...
## Phase 1 progress
**Step 1 SHADOW shipped 2026-07-23:** new `api/agent/preset-master.js` (resolvePresetKey via offer stamp w/ 5-min cache; masterEdge compiled from PRESETS; shadowCompareEdge log-only + deduped, treats enabled=false as tier-2 pause not drift). Wired fire-and-forget into `resolveEdge` (api/agent/_router.js) - the one seam all routing flows through. Zero behavior change.
**OFFLINE FULL DIFF (prod, 2026-07-23): master vs DB = GTA 23/23 exact, San Jose 23/23 exact, Detail 0/23 (has NO edge rows - runs hardcoded fallbacks; flip GIVES Detail a working graph).** Flip is provably safe.
**Step 2 FLIP: LIVE since 2026-07-23 (PR #1565).** Zoran skipped the quiet-log wait: "flip now, Elijah is the guardrail through Hawkeye + breadcrumbs." resolveEdge serves the MASTER first; DB wins only as tier-2 pause (enabled=false) or fallback (no stamp / no master edge). Shadow reversed: `[preset-shadow]` now flags DB drift from master. Emergency off: env `PRESET_EDGE_SOURCE=db` + redeploy, or git revert. Detail Miami silently upgraded from hardcoded fallbacks to the real master graph.
**PHASE 3 CLEANUP DONE 2026-07-23:** (1) applyPreset writes ONLY pipeline_stages identity anchors - edge copy-stamping + the 409/needs_force/force flow deleted (force param accepted+ignored); (2) config view renders the flow from the MASTER via new `GET api/offers/apply-preset?action=graph&client_id=` (pause overlay from stage_transitions rows client-side; API-down fallback = raw rows); (3) Exit-editor add/edit-destination/delete RETIRED in client-portal (tier-1 fork hatch closed) - only the pause toggle remains (updates all matching rows, or inserts an override row for post-Phase-3 academies with synthetic `m:` ids); (4) `seed_default_stage_transitions()` DROPPED from prod (migration 20260723200000); (5) setup-status `transitions` = master edge count when stamped. stage_transitions ROWS deliberately kept at the time: emergency fallback (PRESET_EDGE_SOURCE=db) + pause overlay. **DELETED 2026-07-24** (migration 20260724170000, applied): all 46 rows were is_seed=true AND enabled=true, i.e. byte-identical duplicates of the master with ZERO pause overrides and ZERO hand-authored edges. Delete is GUARDED (only is_seed AND enabled) so any future pause row survives. Table is now empty; the master is the single source. Recovery if a master-reader bug ever appears: run bam-portal/supabase/restore/stage_transitions_backup_20260724.sql (exact rows, ON CONFLICT DO NOTHING), then PRESET_EDGE_SOURCE=db + redeploy. Verified after deleting: all 3 academies still stamped free_trial, masterEdge serves every probed edge.
...
**✅ PHASE 0 FULLY COMPLETE 2026-07-23.** Code merged to main via PR #1560, prod deploy Ready (12m), then the data cleanup ran as migration `20260723143000_finish_ghosted_role_cleanup` (guarded + idempotent; deletes duplicate `interested` rows that have no opps and whose client already has `ghosted`, then renames the rest). VERIFIED AFTER: zero `interested` rows anywhere (pipeline_stages / opportunities / stage_transitions both directions); GTA orphan gone; San Jose's 17 leads moved cleanly onto `ghosted`; GTA + San Jose each hold 23 edges with went_quiet->ghosted, ghosted->replied->responded and nurture->replied->responded all intact. `cc_qualified_trials` latest-outcome-wins live (migration 20260723134812), GTA close rate unchanged at 4 won / 9 lost / 5 pending.

Historical note (the gate that mattered): Prod still runs code that authors `interested`, so renaming rows now just re-drifts (that is exactly how these orphans appeared) and could duplicate a stage row for San Jose mid-import. Run IMMEDIATELY AFTER merge+deploy, in this order (GTA delete FIRST - otherwise the global rename collides with GTA's existing `ghosted` row on the (client_id, role) unique):
```sql
...
- Pricing transparency MODE: re-scoped 2026-07-24 to the AGENT TEMPLATE (`AGENT_TEMPLATES` in api/agent/presets.js), not the preset, so a template reused in another sales system carries its disclosure policy along. Needs a `clientId + runtime -> template` resolver that does not exist yet. The pricing FACT is now numbers only (from routable `offer_prices`); disclosure lives in the shared `core_behavior`. See docs/agent-pricing-transparency-plan.md.
