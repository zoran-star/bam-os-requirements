...
**Step 2 FLIP (next):** resolveEdge serves master-first (DB fallback + per-academy pause overrides win). Watch `[preset-shadow]` lines in Vercel logs for a few days first - MISSING-IN-DB for Detail is expected. Then swap board/config-view stage rendering onto the master too. Step 3 = Phase 3 cleanup (retire stage_transitions + apply-preset copy-stamping).
