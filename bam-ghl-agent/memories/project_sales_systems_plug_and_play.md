...
A lead sent to Nurture is marked LOST. When they reply and bounce Nurture -> Responded, the Lost mark MUST come off - otherwise the booking agent ignores them forever (agents gate on OPEN opps: `isOpenOpp` in api/agent/_stage.js filters status=open only). Verify the nurture-reply/re-entry path resets opportunity status to open; fix if it doesn't. Slot into Build 1/Phase 0 verification.
