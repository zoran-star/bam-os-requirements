...
`_positionSpotlight(step)` first **scrolls the target to the viewport centre** (`scrollIntoView({block:'center'})`, instant) so a target below the fold isn't left off-screen, then reads `getBoundingClientRect()` and positions the tooltip on `top` / `bottom` / `left` / `right`. Auto-flips if it would clip the viewport (e.g., `top` flips to `bottom` if there's no room above). Clamps to viewport edges with 12px padding. Reposition fires on resize + scroll. (The scroll step was added 2026-05-22 — steps 5 + 7 had been spotlighting buttons that sat below the fold.)

### Demo data injection
`TOUR_DEMO_CONTAINERS` config lists 3 containers (`#ticket-list-live`, `#marketing-request-list-pending`, `#marketing-campaigns-container`). Each container's `innerHTML` is stashed in `dataset.tourOriginal` and replaced with the demo; originals are restored on tour close. `_tourSetupDemos()` is re-run before every spotlight step (added 2026-05-22) so a late data fetch can't wipe a demo target. The campaign demo injects unless a `.btn-change-campaign` already exists (`onlyIfMissing` — was `.campaign-card`, but sample campaign cards have no Change button, so step 6 ended up with no target at all; fixed 2026-05-22). Demo elements carry `class="tour-demo"` for inspector clarity.
