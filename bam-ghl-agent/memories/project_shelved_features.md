...
| Tower (GM agent) bar + boardroom focus | NOT built yet - design decided with Cole 2026-07-06: Sage-style bar on Home v2 between the win banner and the cards (prototype SageBar styling, typewriter prompts), opens a BOARDROOM focus room using the members depth-popup idiom (`_ccOpenFocusPage` + `mm-depth`) but wider (~1150px card), no roster rail - instead a "The floor" briefing rail with one-line live statuses for Hawk/Flare/Shield, Tower routes specialist questions with handoff chips into each agent's surface | Build it when the real LLM agent endpoint lands (clone the `_mma*` structure with `gma-` ids; extend `#membersMachineModal` depth CSS selectors, `_ccOpenFocusPage` depth toggle, `_ccFocusCloseOpen`, `_ccFocusHud` label) | 2026-07-06 | Cole: hide the GM until it's wired up (member agent is still a rule-based stub; don't ship a second stub) |
