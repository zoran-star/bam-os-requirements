...
## Sales preset (Gap #2) — STATION MODEL since 2026-07-14
...
**2026-07-25 (Build T of the money model):** sales tax is now an academy TEMPLATE, not free text. `clients.tax_config` = `{label, pct}` (NULL = no tax), edited in Blueprint > General > Sales tax via the `update_client_basics` RPC. Each offering + commitment carries `taxable: "Yes"/"No"` in `offers.data.pricing` instead of the old `added_fees` / `added_fees_description` free-text pair (those fields stay in existing JSON but are read ONLY when the academy has no `tax_config`). All money math goes through `api/_fees.js resolveFee({taxConfig, taxable, legacyText})`; `match-prices.js` fetches the template and passes it per row. Setting the template IS the opt-in, so unset rows count as taxable and "No" is the per-row exemption. GTA migrated (HST 13% + taxable Yes on every Membership row) with catalog amounts unchanged: $226.00 / $315.27 / $850.89 verified. Plan + risks: `docs/money-model-plan.md`.

**2026-07-25 (Build S of the money model): one-time SIGN-UP FEE.** New offering fields in the wizard Pricing step: `signup_fee` (currency, pre-tax), `signup_fee_taxable` (Yes/No, independent of the plan), `signup_fee_on_base` (Charge/Waive), plus `signup_fee_charge` (Charge/Waive) per commitment. Charge/waive is EXPLICIT per option; anything unanswered charges nothing. PER ATHLETE (one checkout = one athlete, so siblings pay per enrollment naturally) and charged again on re-enrollment, both Zoran's calls.

The fee is a REAL catalog row keyed `<plan>|signup_fee`, term `one_time`, flowing through the normal pipeline (match-prices -> create-price -> pricing_catalog -> offers-sync -> offer_prices) so it gets its own Stripe product. `create-price.js termToInterval` now returns `recurring:null` for one_time, and stripeFetch drops nulls, so it mints a genuine one-time price. **A fee row is quotable + rider-chargeable but NEVER standalone-sellable:** `website/offer.js` filters it out of `purchasable` (exposes `signup_fees` + per-card `signup_fee_cents`), `website/checkout.js` 400s if the selected price IS the fee and otherwise attaches it via `add_invoice_items` (index 1 when a future-start line already took 0) on the FIRST invoice only, and `fact-render renderPricing` splits it out of the term grouping or it renders as a fake commitment length.

Also: `dep: { notEmpty: true }` is a new field-config shape (show once the parent field has a value), supported in all three dep evaluators + the dep badge.
...
RE-ISSUE: a Stripe coupon's `applies_to` is IMMUTABLE (docs: only name/metadata/currency_options are updatable) and a promotion code cannot be repointed, so `create-discount` compares the live coupon's `applies_to.products` to the owner's list and returns `needs_reissue`; with `reissue:true` it creates a new restricted coupon, deactivates the old promo code, and mints a new one with the same string. Deactivation stops future redemptions only, so live members keep their discount. Before this, the Stripe pill returned "exists" and did nothing, making applicability unreachable for any already-live code.
