...
**2026-07-05 - v1.0 shipped.** Canonical system extracted from the three "good skin" V2 pages (Home `renderHomeV2`, Assets, Calendar) in `bam-portal/public/client-portal.html`.
...
Other views still on legacy radii/hardcodes (disperse per-view); Home KPI catalog + Hawkeye feed emoji icons; Assets alert()/prompt() feedback; staff portal `src/tokens/tokens.js` separate palette (open decision).
