...
**2026-07-27 staff Content pass (Cole):** ContentView (biggest file, 2.9k lines) - 17 alert()s + window.confirm + BOTH homegrown banner systems replaced by shared dialogs.jsx (hosts mount inside renderMainTabs' fragment), 13 emoji glyphs → SVG icon kit (_ctIco), 19 radius-6 → 8, realtime on `content_tickets` (quiet refetch + new-request toast), `?cticket=` deep link, channel chips now show active counts, "Due today" + "My queue" (open tickets assigned to me, cross-tab) quick chips, overdue dots on sub-tabs, avatars on queue rows (marketing.js already returned brand_data), skeletons, the 3 outward "send" handoffs (review/systems/marketing) get context confirms with finals count + academy, upload failure alert → toast mentioning TUS resume, GuideEditor gets a JSON-snapshot dirty guard + Esc (long-form guide writing can't be lost silently).

**2026-07-27 staff Team pass (Cole):** TeamView + StaffModals - **booking_url + avatar_url are now editable in the staff edit modal** (api/clients.js update-staff accepts both, http(s)-validated, empty clears; closes the "Ximena's booking link only settable via SQL" open item from project_action_items.md - update that note when the link is actually set). Cards: avatar photos w/ initials fallback, "N academies" count on SM cards (clients grouped by scaling_manager_id), amber "No booking link" chip on SM/marketing roles, role-capability hint line under each group header (ROLE_HINTS - keep in sync with api/_roles.js), search box, skeletons, banner → shared toasts, glyphs cleaned. Both staff modals: Esc + overlay close with dirty guards.
