...
- **2026-07-24: WIZARD-SIDE MARKER SHIPPED (the fast-follow, loop now closed both ways).** `_bbAgentSectionTips` + `_bbAgentBadge()` in client-portal.html (next to `_bbSections`) render a blue `Powers your sales agent` pill on the 5 offer-wizard sections that feed a rendered fact: general_info, schedule, value, pricing, policy. Sales/onboarding and every NON-training offer type get nothing (derivedFactOverrides only reads type=training). `title` tooltip per section ("Your agent reads this when a parent asks about pricing."). Placement: its OWN row above the fields, NOT inside `.bb-wizard-step-head` - the onboarding flow hides that header (`#obf-overlay #obf-bb-host`), and a new academy fills its offer in THERE, so a header badge would be invisible exactly where it matters. CSS is deliberately unscoped (the wizard mounts both inside `#view-blueprint` and in the body-level obf overlay). Colour `#6EA8D8` = the brain view's "LIVE - from your offer" badge. **Keep the map in sync with FACT_SOURCES / derivedFactOverrides - if a renderer starts or stops reading a section, change both in one commit.**
- **2026-07-24 (Builds 1+2 of the pricing plan): pricing now renders from `offer_prices`, NOT the offer.** The offer stores PRE-TAX amounts beside a free-text `added_fees` note; `offer_prices` stores the exact total Stripe bills. GTA's live agent had been quoting $200/$279 for plans that charge $226.00/$315.27. `renderPricing(data, prices)` now takes the routable+active rows (numbers) and reads the offer only for copy, what's included, commitment `after`, discount_notes and discount_codes (the last of which the shared objection text had always told the agent to cite but never gave it). Empty catalog -> the agent quotes NOTHING and flags admin (San Jose today, deliberate); a failed read -> null so it falls back instead of asserting. ALSO: the hardcoded `pricing` default in prompt-structure.js held GTA's own $185-$565 ladder and is a LIVE fallback, not dead code - Shig Hoops (V2, no offer, no stored row) was carrying it. Replaced with the shared `PRICING_NOT_CONFIGURED` constant. Disclosure moved to `core_behavior` item 4 (still RANGE for everyone, behavior unchanged); the 3 dangling 'follow its transparency mode' references were rewritten. Plan + remaining builds: `docs/agent-pricing-transparency-plan.md`.
- Still open in Build 2: social_proof/Google (Build 5 - then websites tie-in); wizard-side 'powers your agent' pills: SHIPPED (see the marker entry above). Pricing transparency MODE is DONE (Builds 3+4, same day): `AGENT_TEMPLATES[...].disclosure` in presets.js, modes `range`/`exact`/`withhold`, resolved per academy by `resolveDisclosureOverride` and rendered as the new `pricing_disclosure` master section. The `clientId + runtime -> template` join it needed is `templateForRuntime` (presets.js) + `resolveAgentTemplate` (preset-master.js) - Phase 4 lesson-scoping should use the same join instead of building its own. Build 6 surfaces it: both brain endpoints return `pricing_disclosure` with `scope: "policy"` + `editable:false` + a `policy` object {mode, template}, and both portals render a locked card with a gold mode badge. Read-only for STAFF too, because the policy is applied after stored rows, so an editable box would save something the prompt builder ignores. **Bug fixed in passing:** `api/agent-sandbox.js` returned no `scope` at all, so the staff brain editor showed derived facts as editable textareas and staff edits were silently discarded at prompt-build time (SandboxApp's read-only branch existed but never fired). The sandbox endpoint now matches agent-train's shape.
...
| pricing | routable+active `offer_prices` for the NUMBERS (what checkout charges); offer.data.pricing for copy, what's included, commitment `after`, discount codes. Never the offer's own price fields. |
...
- PRESET-level (shared, a change hits ALL academies): trial link, qualification. Pricing-transparency MODE was re-scoped 2026-07-24 to the AGENT TEMPLATE rather than the preset, so reusing e.g. `trial_booking` in another sales system carries its disclosure policy along.
