...
- Still open in Build 2: social_proof/Google (Build 5 - then websites tie-in). Wizard-side 'powers your agent' pills: SHIPPED (see the marker entry above). Pricing transparency MODE: re-scoped 2026-07-24 to the AGENT TEMPLATE, not the preset, so it travels when a template is reused - needs a `clientId + runtime -> template` resolver (Builds 3+4 of the pricing plan).
