...
| Provider-specific product data | Canonical Offer/OfferPrice plus integration mapping | `partial` | Parent-app `membership_plans` become OfferPrice projections: Business Blueprint `offers.data.pricing.pricing_offerings` + `pricing_catalog` canonical Stripe/CoachIQ mapping. `pricing_catalog` now has confirmed rows with `offer_id`, `offer_price_key`, and `stripe_price_id`; Offers still lack structured credit entitlement fields. Lineage columns are soft references for now, but core should eventually adopt them because Offers are the FC product direction. |
