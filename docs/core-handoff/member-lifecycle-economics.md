...
| Fee suppressed for academies with discount codes | Until coupon applicability (Build C) ships, a sub-level Stripe coupon discounts every invoice line including the fee | Remove the gate once per-line coupon targeting exists |
