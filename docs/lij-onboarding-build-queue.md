...
| 21 | BAM NY has a broken half-seed: ghosted/nurture automation rows exist with ZERO steps - re-seed after PR #1601 merges | FRICTION | queued | Found by the preset session's divergence data. Not SJ, but same re-seed motion as SJ's post-merge apply |
| 22 | Remaining Toronto hardcodes in the AI booking lane: `agent-approvals.js:294,364` (timezone passed into runAgent/runOpener) + `agent/booking.js:138,160` (default param) - the AI thinks in Toronto time when booking SJ trials. Also: tester must verify the `QUIET_TZ` constant in `_quiet.js:7` really is only a null-default (SJ's tz IS set, so quiet hours should be fine, but the SIGN UP FEE session claims otherwise - settle it with evidence) | BLOCKER | blocked on #1587 merge | Files are on the SIGN UP FEE session's do-not-touch list; coordinate with them before touching. Same class of bug as item 4, next in the tz series |
