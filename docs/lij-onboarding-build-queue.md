...
**PR #1587 is CONFLICTED** (2026-07-26): merge attempted on Zoran's approval, GitHub rejected. Main moved when #1601 landed. Owner session asked to rebase, preserving its load-bearing `loadMergedOverrides(clientId, "confirm")` arg and #1601's `clientVars`/`locFor` work. Zoran HAS approved the merge and the 2SIBLING discount going live; merge as soon as it is mergeable. Verified before approving: the GTA price change is exactly +13% HST (200->226.00, 279->315.27, 753->850.89), not a price rise.
...
| 40 | **Lij's email mismatches across records**: `client_users.email` = elijah@3dsportsprep.com but `clients.email` = elijah@byanymeanssanjose.com. Decide which is canonical before anything emails or notifies him | FRICTION | queued | 3dsportsprep.com looks like a prior/other business. Confirm with Lij |
